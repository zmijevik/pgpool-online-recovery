...
ReattachDegeneratedSlave () {
    ...
    ssh -t -i $postgres_user_key postgres@$current_slave_name "sudo systemctl restart postgresql"
...
SwitchOldMasterToSlave () {
    ...
    ssh -t -i $postgres_user_key postgres@$new_slave_name "sudo systemctl stop postgresql"
    ...
    ssh -t -i $postgres_user_key postgres@$new_master_name "sudo systemctl restart postgresql"
    ...
    ssh -i $postgres_user_key postgres@$new_slave_name "bash /var/lib/pgsql/streaming-replication.sh $new_master_name"
...
current_master_state=$(pcp_node_info 10 localhost 9898 $pgpool_username $pgpool_password $current_master_id | cut -d' ' -f3)
current_slave_state=$(pcp_node_info 10 localhost 9898 $pgpool_username $pgpool_password $current_slave_id | cut -d' ' -f3)
